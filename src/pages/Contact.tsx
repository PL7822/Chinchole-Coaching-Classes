    ...
    <div>
      <section className="bg-[image:var(--hero-gradient)] py-16 text-primary-foreground">
        <div className="container mx-auto px-4 text-center">
          <h1 className="font-heading text-3xl font-extrabold md:text-4xl">Contact Us</h1>
      ...
      <section className="py-16">
        <div className="container mx-auto grid gap-10 px-4 md:grid-cols-2">
          {/* Contact Info */}
          <div>
            <h2 className="mb-6 font-heading text-2xl font-bold text-primary">Get In Touch</h2>
            <ul className="space-y-5">
              {[
                { icon: Phone, label: "Phone", value: "+91 98765 43210", href: "tel:+919876543210" },
                { icon: Mail, label: "Email", value: "info@chincholecoaching.com", href: "mailto:info@chincholecoaching.com" },
                { icon: MapPin, label: "Address", value: "Palam, Maharashtra" },
                { icon: Clock, label: "Abhyasika Timing", value: "6:00 AM – 10:00 PM" },
              ].map((c) => (
                <li key={c.label} className="flex items-start gap-4">
                  ...
                  <div>
                    ...
                    {c.href ? (
                      <a href={c.href} className="text-sm font-semibold text-foreground hover:text-secondary">
                        {c.value}
                      </a>
                    ) : (
            ...
            {/* Map placeholder */}
            <div className="mt-8 flex h-48 items-center justify-center rounded-xl border-2 border-dashed border-border bg-muted">
              <p className="text-sm text-muted-foreground">📍 Google Map – Palam, Maharashtra</p>
            </div>
          </div>

          {/* Inquiry Form */}
          <div>
            <h2 className="mb-6 font-heading text-2xl font-bold text-primary">Send an Inquiry</h2>
            {submitted ? (
